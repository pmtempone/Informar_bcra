...
Asistencia_crediticia <- rep(as.character("09"),nrow(info_bcra_sdc))
...
importes$Asistencia_crediticia <- as.character(importes$Asistencia_crediticia)
importes <- importes %>% filter(Importe>0.5)

importes$Importe <- str_pad(importes$Importe,12, pad = "0")
